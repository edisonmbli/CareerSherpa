...
长期维护方案（客观评估） 当前代码质量不是“烂”，但确实存在明显的结构性风险，导致小改动容易引发连锁问题：

- 状态来源过多：服务端状态、v2 store 状态、tabValue、localStorage 同时参与 UI 判断，导致“条件重叠 + 竞争”。
- 副作用过强：tab 自动切换、refresh、reset 都在 UI 层触发，一旦条件错判就会把状态带偏。
- 缺少明确的生命周期边界：服务切换、会话切换、任务切换这三个概念目前混在同一段逻辑里。
  建议的“长期可运维方案”

- SSOT（单一事实源） ：统一以服务端 ExecutionStatus + 子任务状态驱动 UI，避免在组件内推断流程。
- 生命周期分层 ：明确三种阶段：
  1. serviceId 变化 → 全量 reset
  2. executionSessionId 变化 → 仅在非活跃状态下 reset
  3. 任务进度变化 → 仅更新状态，不重置 UI/Tab
- 集中化推导 ：继续强化 deriveStage 作为唯一阶段推导入口，所有 UI 使用同一输出。
