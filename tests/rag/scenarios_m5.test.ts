...
async function retrieveWithFallback(
  ...
  items: UnifiedRagItem[]
  used: 'vectorStore' | 'prismaFallback'
  debug?: string
}> {
  const { lang, topK = 5, minScore = 0 } = opts
  try {
    const res = await queryRagByCategory(query, category, {
      lang,
      topK,
      minScore,
    })
    const items: UnifiedRagItem[] = res.map((r) => {
      const normalizedLang =
        r.lang === 'zh' ? 'zh' : r.lang === 'en' ? 'en' : lang
    ...
    try {
      ...
      const rows = await findSimilarKnowledgeEntries(
        embedding,
        { lang, category, isPublic: true },
        topK,
        minScore,
      )
      ...
      // 保留原始错误链路，便于定位
      throw new Error(
        `RAG 检索失败（vectorStore & Prisma 均失败）：${msg} | ${msg2}`,
      )
    }
  }
}
...
describe('M5 场景覆盖：queryRag', () => {
  it('场景1：match（zh）命中 Part3+Part4，排除面试内容', async () => {
    if (!(hasZhMatch || hasZhMatchIntent)) {
      console.warn(
        '[M5 Debug] 跳过原因：zh 的 job_match/express_job_intent 分类计数为 0',
      )
      return
    }
    const query = '分析 JD 和简历的匹配度'

    const part3Res = await retrieveWithFallback(query, 'job_match', {
      lang: 'zh',
      topK: 3,
      minScore: 0,
    })
    const part4Res = await retrieveWithFallback(query, 'express_job_intent', {
      lang: 'zh',
      topK: 3,
      minScore: 0,
    })
      ...
      JSON.stringify(
        {
          ...
          results: {
            ...
            express_job_intent: part4.map((x) => ({
              id: x.id,
              title: x.title,
              score: x.score,
              category: x.category,
  ...
  it('场景2：customize（zh）命中 X-Y-Z，排除职业锚', async () => {
    if (!hasZhCustomize) {
      console.warn('[M5 Debug] 跳过原因：zh 的 cv_customize 分类计数为 0')
      return
    }
    const query = '如何量化我的工作成就'
    const resPack = await retrieveWithFallback(query, 'cv_customize', {
      lang: 'zh',
      topK: 5,
      minScore: 0,
    })
  ...
  it('场景3：interview（en）命中弱项回答与 P-P-F，排除 JD 拆解', async () => {
    if (!(hasEnInterviewIntro && hasEnInterviewQa)) {
      console.warn(
        '[M5 Debug] 跳过原因：en 的 self_introduction/interview_strategies 分类计数为 0',
      )
      return
    }
    const query = 'how to answer about my weakness'
    const introPack = await retrieveWithFallback(query, 'self_introduction', {
      lang: 'en',
      topK: 3,
      minScore: 0,
    })
    const qaPack = await retrieveWithFallback(query, 'interview_strategies', {
      lang: 'en',
      topK: 3,
      minScore: 0,
    })
      ...
      JSON.stringify(
        {
          input: {
            query,
            lang: 'en',
            categories: ['self_introduction', 'interview_strategies'],
          },
          used: {
            self_introduction: introPack.used,
            interview_strategies: qaPack.used,
          },
          results: {
            self_introduction: intro.map((x) => ({
              id: x.id,
              title: x.title,
              score: x.score,
              category: x.category,
