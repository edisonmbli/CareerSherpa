...
describe('sse-event-processor', () => {
  describe('processSseEvent', () => {
    ...
    describe('result events', () => {
      ...
      it('should extract summary result', () => {
        ...
        expect(result?.summaryResult).toEqual(json)
      })
  ...
  describe('getTokenTarget', () => {
    ...
    it('should return vision for SUMMARY_STREAMING (free tier)', () => {
      expect(getTokenTarget('SUMMARY_STREAMING')).toBe('vision')
    })
    ...
    it('should return null for pending states', () => {
      expect(getTokenTarget('JOB_VISION_PENDING')).toBeNull()
      expect(getTokenTarget('SUMMARY_PENDING')).toBeNull()
      expect(getTokenTarget('MATCH_PENDING')).toBeNull()
    })
  ...
  describe('shouldSwitchTask', () => {
    it('should switch from JOB_VISION_COMPLETED to MATCH_PENDING', () => {
      expect(shouldSwitchTask('JOB_VISION_COMPLETED', 'MATCH_PENDING')).toBe(
        true,
      )
    })

    it('should switch from SUMMARY_COMPLETED to MATCH_STREAMING', () => {
      expect(shouldSwitchTask('SUMMARY_COMPLETED', 'MATCH_STREAMING')).toBe(
        true,
      )
    })
    ...
    it('should not switch within same phase', () => {
      expect(
        shouldSwitchTask('JOB_VISION_PENDING', 'JOB_VISION_STREAMING'),
      ).toBe(false)
    })

    it('should not switch backwards', () => {
      expect(shouldSwitchTask('MATCH_STREAMING', 'JOB_VISION_PENDING')).toBe(
        false,
      )
    })
  })
