...
type StreamMode = 'vision' | 'ocr' | 'summary' | 'preMatch' | 'match' | 'error' | 'init' | 'waiting'
...
function deriveMode(
    ...
    if (tier === 'paid') {
        if (status === 'OCR_PENDING' || status === 'OCR_COMPLETED') {
            return 'ocr'
        }
        if (status === 'SUMMARY_PENDING' || status === 'SUMMARY_COMPLETED' || status === 'SUMMARY_STREAMING') {
            return 'summary'
        }
        if (status === 'PREMATCH_PENDING' || status === 'PREMATCH_COMPLETED') {
            return 'preMatch'
        }
        // Paid tier: During MATCH_PENDING, continue showing preMatch content
        // Only switch to 'match' when streaming or completed
        if (status === 'MATCH_PENDING' && !hasMatchContent) {
            return 'preMatch'
        }
    }
    ...
    if (
        tier === 'free' &&
        (status === 'SUMMARY_PENDING' || status === 'SUMMARY_STREAMING' || status === 'SUMMARY_COMPLETED')
    ) {
        return 'vision'
    }
    ...
    if (
        ...
        ((status === 'MATCH_PENDING' || status === 'MATCH_STREAMING') && hasMatchContent)
    ) {
        return 'match'
    }
...
export function StreamPanelV2({
    ...
    // Check if we have any match content (used to determine when to switch to match mode)
    const hasMatchContent = !!(matchContent || matchJson)

    const mode = deriveMode(status, tier, hasMatchContent)
    ...
    const visionFlush = !!matchJson // Only flush when match result arrives
    const {
        displayedContent: displayedVisionContent,
        isTyping: isVisionTyping,
    } = useTypewriterBuffer({
    ...
    const ocrFlush = !!summaryJson
    const {
        displayedContent: displayedOcrContent,
        isTyping: isOcrTyping,
    } = useTypewriterBuffer({
    ...
    const summaryFlush = !!preMatchJson
    const {
    ...
    const {
        displayedContent: displayedMatchContent,
    ...
    useEffect(() => {
        const viewport = scrollerRef.current?.closest('[data-slot="scroll-area-viewport"]')
        if (viewport) {
            requestAnimationFrame(() => {
                viewport.scrollTop = viewport.scrollHeight
            })
        }
    }, [displayedMatchContent, displayedVisionContent, displayedOcrContent, displayedSummaryContent, displayedPreMatchContent])
    ...
    const displayContent = useMemo(() => {
        switch (mode) {
            ...
            case 'vision':
                // Free tier: Show JSON if available (flush), otherwise typewriter content
                // Note: Text-only JD in Free tier uses SUMMARY_* statuses and routes to 'summary' buffer
                // So we also check summaryJson/summaryContent as fallback
                if (visionJson) {
                    return JSON.stringify(visionJson, null, 2)
                }
                if (summaryJson && tier === 'free') {
                    // Free tier text-only JD - summary result available
                    return JSON.stringify(summaryJson, null, 2)
                }
                if (displayedVisionContent) {
                    return displayedVisionContent
                }
                if (displayedSummaryContent && tier === 'free') {
                    // Free tier text-only JD - summary content streaming
        ...
        mode, status,
        displayedVisionContent, visionJson,
        displayedOcrContent, ocrJson,
        displayedSummaryContent, summaryJson,
        displayedPreMatchContent, preMatchJson,
        displayedMatchContent, matchJson,
        errorMessage, dict
    ])

    // Show cursor only during streaming
    const showCursor = mode === 'match' && status === 'MATCH_STREAMING' && !matchJson
