...
export function KnowledgeRefreshModule({
  ...
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-lg sm:rounded-xl bg-stone-50 dark:bg-stone-900/40 border border-stone-200/60 dark:border-white/10 shadow-sm backdrop-blur-sm',
        matchThemeClass,
        className,
      )}
    >
      ...
      <div className="relative z-10 p-5 md:p-6 space-y-6">
        <div className="flex items-center gap-3">
          <div className="relative inline-block">
            ...
            <h4 className="text-[22px] leading-[30px] font-bold font-[family-name:var(--font-playfair),serif] text-stone-900 dark:text-stone-50 tracking-tight relative">
              {finalLabels.title}
            </h4>
          </div>
        ...
        <div className="space-y-6">
          {topics.map((topic, index) => (
            <div key={index} className="space-y-3">
              <h5 className="text-base font-semibold text-stone-900 dark:text-stone-100">
                {topic.topic}
              </h5>
              <p className="text-sm text-stone-600/80 dark:text-stone-300 pl-3 border-l-2 border-stone-200 dark:border-stone-700 leading-relaxed">
                {topic.relevance}
              </p>
              <ul className="space-y-2">
                {topic.key_points.map((point, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-2 text-sm text-stone-900/80 dark:text-stone-300"
                  >
