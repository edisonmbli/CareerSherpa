...
export function ExpertVerdict({ content, label }: ExpertVerdictProps) {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg sm:rounded-xl bg-stone-50 dark:bg-stone-900/40 border border-stone-200/60 dark:border-white/10 shadow-sm backdrop-blur-sm">
      {/* Quote Watermark - Replaces Noise Texture */}
      <Quote className="absolute -top-4 -left-4 w-24 h-24 text-stone-900/5 dark:text-stone-100/5 rotate-180 z-0 pointer-events-none" />
      ...
      <div className="relative z-10 p-5 md:p-6">
        ...
        <div className="prose prose-lg max-w-none dark:prose-invert relative z-10">
          ...
          <ReactMarkdown
            components={{
              p: ({ node, ...props }) => (
                <p
                  className="font-[family-name:var(--font-noto-serif),serif] text-xs md:text-sm leading-relaxed text-stone-700 dark:text-stone-200 first-letter:float-left first-letter:text-4xl first-letter:font-bold first-letter:mr-2 first-letter:mt-0.5 first-letter:text-stone-900/40 dark:first-letter:text-stone-100/40 first-letter:font-[family-name:var(--font-playfair),serif]"
                  {...props}
                />
              ),
              strong: ({ node, ...props }) => (
                <span
                  className="font-medium text-stone-900 dark:text-stone-100 bg-stone-100 dark:bg-stone-800 px-1 rounded mx-0.5 box-decoration-clone border border-stone-200 dark:border-stone-700/50"
                  {...props}
                />
              ),
        ...
        <div className="mt-4 flex justify-end opacity-40">
          <div className="h-px w-16 bg-stone-900 dark:bg-stone-100" />
        </div>
      </div>
    </div>
  )
