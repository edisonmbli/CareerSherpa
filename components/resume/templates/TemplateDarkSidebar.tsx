...
export function TemplateDarkSidebar({ data, config }: TemplateProps) {
  ...
  const renderSection = (key: string, isSidebar: boolean) => {
    ...
    const subHeaderClass = isSidebar
      ? 'text-white font-semibold'
      : 'text-gray-900 font-bold text-lg'
    const dateClass = isSidebar
      ? 'text-gray-400 text-xs block mt-1'
      : 'text-gray-600 text-sm font-medium'
    const companyClass = isSidebar
              ...
              <div className="space-y-4">
                {data.educations.map((item) => (
                  <div key={item.id}>
                    ...
                    <div
                      className={
                        isSidebar ? 'text-gray-300 text-sm' : 'text-gray-700'
                      }
                    >
        ...
        return (
          data.skills && (
            <section key={key} className="mb-8">
              ...
              <div className={textClass}>{renderDescription(data.skills)}</div>
            </section>
          )
        )
      case 'certificates':
        ...
        return (
          data.hobbies && (
            <section key={key} className="mb-8">
              ...
              <div className={textClass}>{renderDescription(data.hobbies)}</div>
            </section>
          )
        )
      case 'customSections':
      ...
      <main className="w-[68%] p-10 bg-white text-gray-900">
        <header className="mb-10">
          <h1
            ...
            } font-bold text-slate-900 tracking-tight mb-2`}
          >
            {basics.name}
          </h1>
