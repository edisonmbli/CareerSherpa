...
import { logError as writeErrorLog, logInfo } from '@/lib/logger'
...
export class ErrorHandler {
  ...
  private detectErrorCode(error: Error | string): string {
    ...
    if (
      lowerMessage.includes('too many pending') ||
      lowerMessage.includes('concurrent limit')
    ) {
    ...
    if (
      ...
      (lowerMessage.includes('resume') || lowerMessage.includes('job'))
    ) {
    ...
    switch (processedError.userFriendlyError.severity) {
      ...
      case 'error':
        writeErrorLog({
          ...
          severity: processedError.userFriendlyError.severity,
          message: logMessage,
          error: processedError.originalError || logMessage,
          ...(processedError.context?.requestId
            ? { reqId: processedError.context.requestId }
            : {}),
          ...(processedError.context?.userId
            ? { userKey: processedError.context.userId }
            : {}),
          ...(processedError.context?.serviceId
            ? { serviceId: processedError.context.serviceId }
            : {}),
