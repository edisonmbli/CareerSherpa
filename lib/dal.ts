...
export async function updateSummaries(params: {
  ...
  ops.push(
    client.resume.update({
      where: { id: resumeId },
      data: {
        resumeSummaryJson: params.resumeSummaryJson,
        totalTokens: params.resumeSummaryTokens,
      },
    })
  )
  ops.push(
    client.jobDescription.update({
      where: { id: jobId },
      data: {
        jobSummaryJson: params.jobSummaryJson,
        totalTokens: params.jobSummaryTokens,
      },
    })
  )
  if (detailedId && params.detailedSummaryJson) {
    ops.push(
      client.detailedResume.update({
        where: { id: detailedId },
        data: {
          detailedSummaryJson: params.detailedSummaryJson,
          totalTokens: params.detailedSummaryTokens,
        },
      })
    )
  }
...
export async function getUserByStackId(stackUserId: string) {
  return getCached(
    ...
    async () => {
      ...
      return client.neonUserSync.findUnique({
        where: { 
          id: stackUserId,
...
/**
 * 确保用户在本地数据库中存在（用于 Stack Auth 同步）
 * 注意：这个函数主要用于确保 Stack Auth 用户在本地有对应记录
 */
export async function createOrUpdateUser(data: {
  ...
  return getCached(
    `user:sync:${data.stackUserId}`,
    60, // 1 min TTL for sync operations
    async () => {
      // 首先检查用户是否已存在
      const existingUser = await client.neonUserSync.findUnique({
        where: { 
          id: data.stackUserId,
          deletedAt: null
        }
      ...
      if (existingUser) {
        ...
        if (data.email && existingUser.email !== data.email) {
          return client.neonUserSync.update({
            where: { id: data.stackUserId },
            data: {
              email: data.email,
              updatedAt: new Date()
            }
      ...
      // 如果用户不存在，这通常意味着 Neon Auth 同步还没有完成
      // 我们返回一个虚拟用户对象，让 Stack Auth 处理实际的用户创建
      console.warn(`User ${data.stackUserId} not found in neon_auth.users_sync. This is expected for new users.`);
...
export async function createTask(data: {
  ...
  return client.task.create({
    data: {
      ...
      inputContextJson: data.inputContextJson,
...
export async function logAudit(data: {
  ...
  // 导入异步审计日志函数
  const { logAuditAsync } = await import('@/lib/audit/async-audit')

  // 使用异步处理，不阻塞主流程
  const success = logAuditAsync({
    userId: data.userId,
    action: data.action,
