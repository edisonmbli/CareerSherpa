...
export async function runGeminiStructured<T>(
    ...
    try {
        ...
        try {
            ...
            console.error('[GeminiDirect] Parse/validation error:', parseError.message)
            console.error('[GeminiDirect] Raw response (first 500 chars):', rawText.slice(0, 500))
            return {
                ok: false,
                raw: rawText,
                ...(usage && { usage }),
...
export async function runGeminiVision<T>(
    ...
    try {
        ...
        try {
            ...
            console.error('[GeminiDirect] Vision raw response (first 500 chars):', rawText.slice(0, 500))
            return {
                ok: false,
                raw: rawText,
                ...(usage && { usage }),
    ...
    try {
        ...
        if (schema) {
            try {
                ...
                console.error('[GeminiDirect] Streaming parse error:', parseError.message)
                return { ok: false, raw: fullText, error: `Parse error: ${parseError.message}` }
            }
        }
