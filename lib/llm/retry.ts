...
export function isRetryableError(
    ...
    if (message.includes('server overloaded') || message.includes('503') || message.includes('service unavailable')) {
        return true
    }
...
function isHardFailure(message: string): boolean {
    const hardFailurePatterns = [
        ...
        'invalid parameter', 'invalid request',
        'json parse', 'parse error', 'json_parse_failed',
...
export async function executeWithSmartRetry<T>(
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        try {
            ...
            console.warn(`[SmartRetry] ${context.templateId} attempt ${attempt + 1}/${config.maxRetries + 1} failed, retrying in ${delay}ms...`, {
