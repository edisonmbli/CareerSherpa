...
import { useWorkbenchV2Store, type WorkbenchStatusV2 } from '@/lib/stores/workbench-v2.store'
import { processSseEvent, getTokenTarget, shouldSwitchTask } from '../ui/sse-event-processor'
...
export function useSseStreamV2(options: UseSseStreamV2Options) {
    ...
    const flushTokenBuffer = useCallback(() => {
        ...
        const currentStatus = useWorkbenchV2Store.getState().status
        const target = getTokenTarget(currentStatus)
    ...
    }, [appendVisionContent, appendOcrContent, appendSummaryContent, appendPreMatchContent, appendMatchContent])
    ...
    const connect = useCallback(
        (taskId: string, fromLatest: boolean = false) => {
            if (!userId || !serviceId || !taskId) {
                sseLog.warn('Cannot connect: missing params', { userId, serviceId, taskId })
                return
            }
            ...
            es.onmessage = (ev) => {
                ...
                try {
                    ...
                    console.log('[SSE DEBUG] Processing Event:', {
                        ...
                        hasResult: !!(processed.visionResult || processed.ocrText || processed.summaryResult || processed.matchResult)
                    })
                    ...
                    if (processed.newStatus) {
                        ...
                        console.log('[useSseStreamV2] Defensive setStatus:', processed.newStatus)
                        ...
                        // Use oldStatus for transition check as we just updated the store
                        const prevStatus = oldStatus
                        const nextStatus = processed.newStatus

                        // ROBUSTNESS: Synthesis of missing COMPLETED statuses for Paid Tier
                        // The backend 'job' stream jumps directly between phases (OCR->SUMMARY->PREMATCH)
                        // without sending explicit COMPLETED events. We must synthesize them to keep UI consistent.
                        if (prevStatus && nextStatus) {
                            // OCR -> Summary transition
                            if (
                                (prevStatus === 'OCR_PENDING' || prevStatus === 'OCR_STREAMING') &&
                                (nextStatus === 'SUMMARY_PENDING' || nextStatus === 'SUMMARY_STREAMING')
                            ) {
                                console.log('[useSseStreamV2] Synthesizing OCR_COMPLETED')
                                setStatus('OCR_COMPLETED')
                            }
                            // Summary -> PreMatch transition
                            else if (
                                (prevStatus === 'SUMMARY_PENDING' || prevStatus === 'SUMMARY_STREAMING') &&
                                (nextStatus === 'PREMATCH_PENDING' || nextStatus === 'PREMATCH_STREAMING')
                            ) {
                                console.log('[useSseStreamV2] Synthesizing SUMMARY_COMPLETED')
                                setStatus('SUMMARY_COMPLETED')
                            }
                            // PreMatch -> Match transition
                            else if (
                                (prevStatus === 'PREMATCH_PENDING' || prevStatus === 'PREMATCH_STREAMING') &&
                                (nextStatus === 'MATCH_PENDING' || nextStatus === 'MATCH_STREAMING')
                            ) {
                                console.log('[useSseStreamV2] Synthesizing PREMATCH_COMPLETED')
                        ...
                        if (shouldSwitchTask(oldStatus, processed.newStatus)) {
                            ...
                            if (processed.nextTaskId && processed.nextTaskId !== currentTaskIdRef.current) {
                    ...
                    if (processed.tokenChunk) {
                        bufferToken(processed.tokenChunk)
                    }
                    ...
                    if (processed.summaryResult) {
                        ...
                        // Content Backfill: If no streaming content received, use result
                        // This triggers the Typewriter effect in StreamPanelV2
                        const state = useWorkbenchV2Store.getState()
                        if (!state.content.summaryContent) {
                            appendSummaryContent(JSON.stringify(processed.summaryResult, null, 2))
                        }
                    }

                    // Debug: trace matchResult handling
                    if (processed.matchResult) {
                        console.log('[useSseStreamV2] matchResult received, calling setMatchResult:', {
                    ...
                    if (processed.preMatchResult) {
                        ...
                        if (!state.content.preMatchContent) {
                            appendPreMatchContent(JSON.stringify(processed.preMatchResult, null, 2))
                        }
                    }
                    ...
                    if (processed.streamCompleted) {
                        console.log('[SSE DEBUG] Stream Completed event received', { taskId })
                        ...
                        if (currentStatus === 'JOB_VISION_STREAMING' || currentStatus === 'JOB_VISION_PENDING') {
                            setStatus('JOB_VISION_COMPLETED')
                        }

                        // Paid Tier Transitions
                        else if (currentStatus === 'OCR_STREAMING' || currentStatus === 'OCR_PENDING') {
                            setStatus('OCR_COMPLETED')
                        } else if (currentStatus === 'SUMMARY_STREAMING' || currentStatus === 'SUMMARY_PENDING') {
                            setStatus('SUMMARY_COMPLETED')
                        } else if (currentStatus === 'PREMATCH_STREAMING' || currentStatus === 'PREMATCH_PENDING') {
                            setStatus('PREMATCH_COMPLETED')
                        }

                        // Match Phase Transitions
                        else if (currentStatus === 'MATCH_STREAMING' || currentStatus === 'MATCH_PENDING') {
            ...
            es.onerror = () => {
                ...
                reconnectTimeoutRef.current = setTimeout(() => {
                    const state = useWorkbenchV2Store.getState()
                    if (!state.connection.isConnected && !isTerminalStatus(state.status)) {
