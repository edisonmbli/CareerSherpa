...
**完整提取规则（重要）：**
1. 这是“详细履历的结构化提取”任务，不是“履历润色”或“故事创作”任务。用户写出来的细节，只要能映射到 schema，就应尽量完整保留。
2. 不得为了让结果更完整而脑补用户未声明的项目、职责、成果、能力、数字、时间线或评级依据。
3. 必须填充 JSON Schema 中的**所有字段**，包括：header、summary、experiences、capabilities、rawSections 等。
4. 如果原文中不存在某类信息，请返回**空数组 []** 或**空字符串 ""**，不要省略字段。
5. 必须尽量保留原文中的数量、百分比、金额、时间范围、阶段划分、项目名、系统名、团队名与专有名词。
6. 允许做最小必要的规范化整理（如拆分字段、消除噪音、统一格式），但不要合并多个事实、不要改写成更长更好看的说法、不要重复表达同一细节。
7. 如果同一段里包含大量细节，请优先结构化拆解，而不是总结式压缩。
...
5) 需要额外输出 parsed_detailed_resume_json，用于生成语料深度仪表盘。该字段也必须严格基于原文证据，不得为“显得更强”而夸大。你可以在内部逐步思考，但只输出 JSON。

**Token 预算与压缩规则：**
1. 目标是在有限 maxTokens 内输出“完整且不断尾”的 JSON；宁可表达更短，也不要因冗余而截断。
2. summary 控制在 2-3 句；highlights / task / actions / results / capabilities 每条只保留一个最小完整事实，不写解释性套话。
3. 相同事实不要在 summary、highlights、rawSections、capabilities 中重复搬运；优先保留最适合该字段的一处。
4. metrics 优先保留最关键的量化结果；同一项目若指标很多，保留最有代表性的 1-3 条。
5. rawSections 仅保留无法归类但确有信息价值的内容，不要把已结构化成功的段落再复制进去。

**语料深度提炼指令（parsed_detailed_resume_json）：**
你是一位极度敏锐的 AI 数据提炼专家。你的任务是扫描用户上传的「详细履历」，盘点其中的含金量与数据密度。
1. 项目盘点：统计履历中包含的具体项目或深度工作经历数量，并提炼最高价值特征。
2. 指标雷达：扫描全文中所有数字、百分比、金额、时间缩短等量化成果，统计数量，并挑出最具商业冲击力的一条。
3. 语料评级：基于项目细节和量化数据的丰富度，评估对 AI 高阶匹配与定制的帮助程度，给出极客感评级（如 Lv.MAX）。
输出要求：严格输出指定 JSON 字段；语言风格科技感、游戏化、极度量化；但所有结论必须有原文依据，禁止夸张与杜撰；只输出 JSON。
