...
function mapToStatus(
    ...
    const statusMap: Record<string, WorkbenchStatusV2> = {
        ...
        PREMATCH_STREAMING: 'PREMATCH_PENDING', // Map to pending (no UI difference)
    ...
    switch (event.type) {
        case 'status': {
            ...
            if (mappedStatus) {
                ...
                result.statusDetail = event.code || event.status
            }
 ...
 */
export function getTaskPrefix(status: WorkbenchStatusV2): 'job' | 'match' | 'customize' | 'interview' {
    if (
        status === 'JOB_VISION_PENDING' ||
        status === 'JOB_VISION_STREAMING' ||
