...
export function queueMaxSizeFor(queueId: string): number {
  ...
  // M10 Logic: Simplified queues mapping
  if (id.includes('paid')) return cfg.queueLimits.paid
  if (id.includes('free')) return cfg.queueLimits.free
...
export function getMaxWorkersForModel(
  ...
  if (id.startsWith('gemini') && tier === 'paid')
...
export function buildUserActiveKey(
  userId: string,
  kind: 'stream' | 'batch',
): string {
